---
title: "IREF - Exploring Risk Gradients"
...
This document contains an attempt to explore how the relative contribution of NCEs to the total predator effect varies across a risk gradient. This exploration is an attempt to flesh out the theory paper and help distinguish it from a rehash of Scott's 2013 paper in Oecologia and ideas in the 2020 Ecology paper.

This is similar to Fig. 4 in the grant proposal, which uses predator density as a proxy for risk.

![Fig. 4 from Grant](IREF_NSFGrant_fig4.png)
...
$$ f(R)= \frac{sR}{1 + shR}$$
where $f(R)$ is the growth curve, $s$ is prey displacement rate, $h$ is handling time, and $R$ is resource density (held constant at $R = 1$).
...
# Range of values used by Scott for Fig. 4 in the grant proposal
k_vec <- c(0, 0.1, 0.13, 0.2, 0.3, 0.4, 0.6, 1, 1.5, 2, 3, 5) 
...
```
**Fig. 2**: The proportion of the total predator effect (TPE) made up of NCEs, as a function of increasing predator density (i.e., predation risk).
...
```{r,echo=FALSE}
ggplot( data = optim.df, aes( x = pred_dens ) ) +
  geom_point( aes( y = optim_CE, colour = "CE" ) ) +
  geom_point( aes( y = 1 - optim_NCE, colour = "NCE" ) ) +
  theme_classic( base_size = 10, base_line_size = 1 ) + 
  ylab("Effect Size") +
  xlab("Predator Density") +
  scale_y_continuous(expand = c(0, 0), limits = c(0, 1.2), breaks = c(0, 0.5, 1) ) +
  scale_x_continuous(expand = c(0, 0), limits = c(0, 6) ) +
  scale_colour_manual(name = "Predator Effect", values = c("red", "blue")) +
  theme( panel.border = element_rect( color = "black", fill = NA, linewidth = 0.75 ), aspect.ratio = 1)
```
...
```{r,echo = FALSE}
ggplot( data = optim.df, aes( x = pred_dens ) ) +
  geom_point( aes( y = 1 - optim_val ) ) +
  theme_classic( base_size = 10, base_line_size = 1 ) + 
  ylab("Optimal Displacement Rate") +
  xlab("Predator Density") +
  scale_y_continuous(expand = c(0, 0), limits = c(0, 1.1), breaks = c(0, 0.5, 1), labels = c(0, 500, 1000) ) + 
  scale_x_continuous(expand = c(0, 0), limits = c(0, 6) ) +
  theme( panel.border = element_rect( color = "black", fill = NA, linewidth = 0.75 ), aspect.ratio = 1)
```
**Fig. 5**: The optimal RITR (displacement rate) as a function of predator density.

---

These findings are probably general in cases where the growth curve is concave-down, while predation risk declines in a linear or saturating way. I would also suggest that the "relative slopes" approach to determining the magnitude of the prey RITR is probably a good rule of thumb, as Scott suggests in Peacor et al. 2013. I.e., prey should only reduce their foraging rates if the slope of the growth curve is greater than the slope of the mortality curve (i.e., the benefits of reducing foraging rates are growing faster than their costs).
...
To explore this, I have taken the encounter rate equation above and nested it inside of a Type II functional response, assuming that encounter rate and attack rate are roughly equal ($Z \approx a$; i.e., an encounter generally leads to an attack in the absence of handling time):
...
We'll start with an (arbitrary) predator handling time ($h_{pred}$) of 5, while holding all other parameters constant (including prey density, but see below), except for the scaling constant applied to predator foraging rates. I adjusted this value from ~1 to 5, because the introduction of handling time to the equation strongly reduces predator feeding rates.
...
# Range of values used by Scott for Fig. 4 in the grant proposal
k_vec <- c(0, 0.1, 0.13, 0.2, 0.3, 0.4, 0.6, 1, 2, 3, 5) 
...
ggplot( data = typeII.df, aes( x = trait ) ) +
  ...
  theme( panel.border = element_rect( color = "black", fill = NA, linewidth = 0.75 ), aspect.ratio = 1 )
```
**Fig. 6**: Growth and mortality curves (blue and red, respectively) as a function of predator density (noted in the header above the plot), when predators exhibit a Type II Functional Response. The optimal RITR is noted using a bar, where the green shaded region is fitness (1 - NCE - CE), the blue shaded region is NCE strength, and the red shaded region is CE strength.
...
ggplot( data = optim3.df, aes( x = pred_dens ) ) +
  ...
  theme( panel.border = element_rect( color = "black", fill = NA, linewidth = 0.75 ), aspect.ratio = 1)
```
**Fig. 7**: The proportion of the total predator effect (TPE) made up of NCEs, as a function of increasing predator density (i.e., predation risk), when predators exhibit a Type II Functional Response.
...
ggplot( data = optim3.df, aes( x = pred_dens ) ) +
  ...
  theme( panel.border = element_rect( color = "black", fill = NA, linewidth = 0.75 ), aspect.ratio = 1)
```
**Fig. 8**: The effect size of NCEs (blue points) and CEs (red points) as a function of predator density at the optimal RITR, when predators exhibit a Type II Functional Response.
...
ggplot( data = optim3.df, aes( x = pred_dens ) ) +
  ...
  theme( panel.border = element_rect( color = "black", fill = NA, linewidth = 0.75 ), aspect.ratio = 1)
```
**Fig. 9**: The optimal RITR (displacement rate) as a function of predator density, when predators exhibit a Type II Functional Response.

---

The relative contribution of the NCE to the total predator effect reaches its peak and declines much sooner. This is because as predator density increases, prey switch from reducing their foraging rates to not responding to predators.

\pagebreak

Next, we'll repeat the above scenarios but instead of manipulating predator density, we will manipulate prey density. This will help us examine how our predictions change with increasing predator satiation.
...
ggplot( data = typeII_full.df, aes( x = trait ) ) +
  ...
  theme( panel.border = element_rect( color = "black", fill = NA, linewidth = 0.75 ), aspect.ratio = 1 )
```
**Fig. 10**: Growth and mortality curves (blue and red, respectively) as a function of prey density (noted in the header above the plot), when predators exhibit a Type II Functional Response. The optimal RITR is noted using a bar, where the green shaded region is fitness (1 - NCE - CE), the blue shaded region is NCE strength, and the red shaded region is CE strength.
...
As prey density increases, the optimal RITR at first shifts to the right, then back to the left. Mathematically, this is probably because as prey density increases, the saturating effect of predator handling time makes it "safer" for prey to be out in the open foraging actively (i.e., because if they do encounter predators, they might not be hungry). But biologically speaking, I don't know how plausible this is. Do individual prey "know" how dense their populations are, and do they have any sense of when/if predators will be satiated? These are population level traits, instead of individual level traits.

\pagebreak

```{r,echo=FALSE}
# Plot % contribution of NCE to total predator effect by predator density
ggplot( data = optim4.df, aes( x = prey_dens ) ) +
  geom_point( aes( y = pct_NCE ) ) +
  theme_classic( base_size = 10, base_line_size = 1 ) + 
  ylab("% NCE contribution to TPE") +
  xlab("Prey Density") +
  theme( panel.border = element_rect( color = "black", fill = NA, linewidth = 0.75 ), aspect.ratio = 1)
```
**Fig. 11**: The proportion of the total predator effect (TPE) made up of NCEs, as a function of increasing prey density, when predators exhibit a Type II Functional Response.
...
ggplot( data = optim4.df, aes( x = prey_dens ) ) +
  ...
  theme( panel.border = element_rect( color = "black", fill = NA, linewidth = 0.75 ), aspect.ratio = 1)
```
**Fig. 12**: The effect size of NCEs (blue points) and CEs (red points) as a function of prey density at the optimal RITR, when predators exhibit a Type II Functional Response.
...
ggplot( data = optim4.df, aes( x = prey_dens ) ) +
  ...
  theme( panel.border = element_rect( color = "black", fill = NA, linewidth = 0.75 ), aspect.ratio = 1)
```
**Fig. 13**: The optimal RITR (displacement rate) as a function of prey density, when predators exhibit a Type II Functional Response.
